...
public class HuffViewer {
	...
	private HuffProcessor myProcessor;
	private RadioButton myCountButton;
	
	public HuffViewer(HuffProcessor processor) {
	...
	private VBox createCompressTab(double contentWidth, double contentHeight) {
		...
		TextField suffix = new TextField(".hf");
		HuffChooser chooser = new HuffChooser(true, "Compress",suffix);
		Button compress = new Button("Compress");
		compress.setOnAction((clicked) -> compress(chooser.getChosenFile(), status, panel, chooser.getSuffix()));
		inputField.getChildren().addAll(chooser.render(), compress);
		Label lab = new Label("suffix");

        	suffix.setPrefWidth(70);
        	
        	ToggleGroup group = new ToggleGroup();
        	RadioButton rbtree = new RadioButton("tree header");
        	myCountButton = new RadioButton("counts header");
        	rbtree.setToggleGroup(group);
        	myCountButton.setToggleGroup(group);
        	rbtree.setSelected(true);
        	
		HBox hb = new HBox(6);
		hb.setAlignment(Pos.CENTER_LEFT);
		hb.getChildren().addAll(lab,suffix); //rbtree, myCountButton);
		inputField.getChildren().addAll(hb,rbtree,myCountButton);
	...
	private VBox createDecompressTab(double contentWidth, double contentHeight) {
		...
		HBox upper = new HBox();
		upper.setMinHeight(80);
		TextField suffix = new TextField(".unhf");
	    suffix.setPrefWidth(70);
		HuffPanel panel = new HuffPanel(contentWidth, contentHeight - 150);
		...
		VBox inputField = new VBox(INTERNAL_PADDING);
		HuffChooser chooser = new HuffChooser(true, "Decompress",suffix);
		Button decompress = new Button("Decompress");
		decompress.setOnAction((clicked) -> decompress(chooser.getChosenFile(), status, panel, chooser.getSuffix()));
		inputField.getChildren().addAll(chooser.render(), decompress);
		Label lab = new Label("suffix");
        
		HBox hb = new HBox(6);
        hb.setAlignment(Pos.CENTER_LEFT);
        hb.getChildren().addAll(lab,suffix);
        inputField.getChildren().add(hb);
	...
	private VBox createCompareTab(double contentWidth, double contentHeight) {
		...
		VBox inputField = new VBox(INTERNAL_PADDING);
		HuffChooser chooserA = new HuffChooser(true, "File A",null);
		HuffChooser chooserB = new HuffChooser(true, "File B",null);
	...
	private VBox createTestTab(double contentWidth, double contentHeight) {
		...
		VBox inputField = new VBox(INTERNAL_PADDING);
		HuffChooser chooser = new HuffChooser(false, "Directory",null);
	...
	private VBox getInfo(double[] times, File[] originalFiles, File[] newFiles) {
		...
		for (int i = 0; i < times.length; i++) {
			...
			double saved = 1.0 - ((double) totalNewLength) / ((double) totalOriginalLength);
	...
	private HuffProcessor getProcessor() {
		return myProcessor;
	}

	private void compress(File original, StatusBar status, HuffPanel panel, String suffix) {
		if (original == null) {
			return;
		}
		...
		File compressed = new File(original.getPath() + suffix);
	...
	private void decompress(File original, StatusBar status, HuffPanel panel, String suffix) {
		if (original == null) {
			return;
		}
		...
		File decompressed = new File(name + suffix);
...
//				out.close();
	...
	public class HuffChooser {
		...
		private TextField mySuffix;

		public HuffChooser(boolean choosingFile, String label, TextField suffix) {
